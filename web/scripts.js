...
function resetFilters() {
    ...
    if (visualizationData) {
        ...
        // Select only "All" options - this ensures only the "All Genres" option is selected
        for (let i = 0; i < genreFilter.options.length; i++) {
            genreFilter.options[i].selected = (i === 0);
        }
        
        // Select only "All" options - this ensures only the "All Artists" option is selected
...
function filterData() {
    ...
    if (includeAllGenres && includeAllArtists) {
        ...
        createVisualization();
        // Update the legend when showing all songs
        createGenreLegend();
        return;
    }
...
function createVisualization() {
    ...
    const layout = {
        ...
        xaxis: { 
            ...
            range: [-8, 8] // Updated x range to ensure proper axis bounds
        },
        yaxis: { 
            title: 'DensMAP Dimension 2',
            range: [-8, 8] // Updated y range to ensure proper axis bounds
        }
    };
