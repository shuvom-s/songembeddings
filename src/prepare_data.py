...
try:
    ...
    print("Warning: lyrics_utils module not found. Limited functionality available.")
    lyrics_utils = None
...
def read_lyrics_for_dataset(df):
    ...
    for _, row in df.iterrows():
        ...
        # Try to get lyrics using our utility if available
        lyric_content = None
        if lyrics_utils:
            lyric_content = lyrics_utils.get_lyrics(artist, track)
        else:
            # Fallback if lyrics_utils not available
            try:
                artist_slug = artist.lower().replace(' ', '-')
                track_slug = track.lower().replace(' ', '-')
                filepath = os.path.join('songs', artist_slug, f'{track_slug}.txt')
                ...
                # Try gzipped file
                elif os.path.exists(f"{filepath}.gz"):
                    with gzip.open(f"{filepath}.gz", 'rt', encoding='utf-8') as f:
                        lyric_content = f.read()
            except Exception as e:
                print(f"Error reading lyrics for {artist} - {track}: {e}")
...
def main():
    ...
    parser.add_argument('--embeddings-file', type=str, default='data/embeddings/liked_songs_embeddings_openai-small.npy',
                       help='Path to embeddings file (.npy)')
    parser.add_argument('--metadata-file', type=str, default='data/embeddings/liked_songs_with_lyrics_openai-small.csv',
                       help='Path to metadata file (.csv)')
    parser.add_argument('--output-file', type=str, default='web/data.json',
    ...
    if args.sae_model:
        try:
            ...
            try:
                ...
                # If not found, try to import from src directory
